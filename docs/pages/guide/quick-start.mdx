...
<Callout>
  **GPTLint supports local LLMs and other providers**, so if you don't want to
  use OpenAI, [see here](./llm-providers.md).
</Callout>
...
<Callout type='warning'>
    ...
    Since [LLM costs can add up quickly](../project/cost.md) for large codebases,
...
to OpenAI, with the vast majority of them using `gpt-3.5-turbo`.
...
For more details on how to interpret the output, see [usage](./usage) and our [faq](/project/faq).

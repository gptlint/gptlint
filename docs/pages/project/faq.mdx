...
The only external service GPTLint relies on is an [LLM provider](/guide/llm-providers), which defaults to [OpenAI](https://openai.com/enterprise-privacy). [OpenAI's enterprise privacy](https://openai.com/enterprise-privacy) is to never store, log, or train your code, and their API is fully SOC 2 compliant.

If you need stronger privacy & security guarantees, GPTLint supports using any [LLM provider](/guide/llm-providers), including [local models](/guide/llm-providers#local-models) which can be self-hosted to guarantee that no data is ever transmitted to any external services.

### What about fine-tuning?

See our notes on [fine-tuning in how it works](./how-it-works#fine-tuning).
